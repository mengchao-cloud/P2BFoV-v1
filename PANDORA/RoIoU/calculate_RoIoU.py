...
class Sph:
    ...
    def sphIoU(self, dets, gt):
        ...
        return final
    # def sphIoU(self, dets, gt):
    #     """修复：球面IoU符号修正（负IoU+1=正确值），保留原计算逻辑"""
    #     if len(dets) == 0 or len(gt) == 0:
    #         return np.zeros((len(dets), len(gt)))
        
    #     d_size, g_size = len(dets), len(gt)
    #     dets_expand = np.repeat(dets, g_size, axis=0)
    #     gt_expand = np.tile(gt, (d_size, 1))
    ...
    #     union = np.clip(union, 1e-10, None)  # 仅保证Union非负，不改动核心逻辑
    #     iou = inter / union
        
        # ===== 核心修复：按你的观察修正符号 =====
        # # 规则：负IoU → 1 + IoU（补集转真实交集）；正IoU → 保持不变
        # iou = np.where(iou < 0, 1 + iou, iou)
        # # 最终确保IoU在[0,1]（防止极端情况）
        # iou = np.clip(iou, 0.0, 1.0)
        
        # return iou.reshape(d_size, g_size)
