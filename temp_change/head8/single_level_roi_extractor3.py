...
class SingleRoIExtractor(BaseRoIExtractor):
    ...
    def bfov_roi_to_14x14_erp(self, bfov_params, erp_width=1024, erp_height=512, grid_size=21):
        """
        Generate 21x21 ERP points from BFOV parameters using PyTorch.
        ...
        Args:
            ...
            grid_size: Grid size, default 21
    ...
    def forward(self, feats, rois, roi_scale_factor=None):
        ...
        if num_levels == 1:
            ...
            sampled_feats = feats[0].new_zeros(N, C, 21, 21)
            
            # Process each ROI individually to save memory
            for i in range(N):
                # Get grid for current ROI (shape: (1, 21, 21, 2))
                ...
                # Sample features for current ROI (shape: (1, C, 21, 21))
            ...
            # Step 5: Pool to 7x7 (21/3=7)
            roi_feats = torch.nn.functional.avg_pool2d(
                sampled_feats, kernel_size=3, stride=3
            )
        ...
        for i in range(num_levels):
            ...
            if inds.numel() > 0:
                ...
                sampled_feats = feats[i].new_zeros(N, C, 21, 21)
                ...
                # 使用detach()断开计算图，避免梯度累积
                dummy_tensor = feats[i].sum().detach() * 0.
                roi_feats += dummy_tensor
