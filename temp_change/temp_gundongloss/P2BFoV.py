...
class P2BFoV(TwoStageDetector):
    ...
    def roll_erp_image_and_annotations(self, img, img_metas, gt_bfov, gt_points, shift_pixels):
        """
        滚动ERP图像和对应的标注
        """
        # 1. 滚动图像
        img_move_view = torch.roll(img, shifts=shift_pixels, dims=3)  # 沿宽度维度滚动
        
        # 2. 滚动图像元数据
        img_metas_move_view = copy.deepcopy(img_metas)
        for meta in img_metas_move_view:
            # 保持原始的三元组格式 (H, W, C)，滚动操作不会改变图像形状
            if len(meta['img_shape']) == 2:
                # 如果已经是二元组，保持原样
                meta['img_shape'] = meta['img_shape']
            else:
                # 保持原始的三元组格式
                meta['img_shape'] = (meta['img_shape'][0], meta['img_shape'][1], meta['img_shape'][2])
            # 添加滚动标记用于后续处理
            meta['is_rolled'] = True
            meta['roll_shift'] = shift_pixels
        ...
        for bfov in gt_bfov:
            if bfov is not None and len(bfov) > 0:
                # 球面边界框滚动：调整经度坐标
                rolled_bfov = bfov.clone()
                # 使用图像元数据中的缩放后尺度计算角度
                img_width = img_metas[0]['img_shape'][1]  # 宽度在img_shape的第二个位置
                shift_angle = shift_pixels * (2 * PI) / img_width
                rolled_bfov[:, 0] = torch.remainder(rolled_bfov[:, 0] + PI + shift_angle, 2 * PI) - PI
                gt_bfov_move_view.append(rolled_bfov)
            else:
                gt_bfov_move_view.append(bfov)
        
        # 4. 滚动点坐标
        gt_points_move_view = []
        for points in gt_points:
            if points is not None and len(points) > 0:
                rolled_points = points.clone()
                # 使用图像元数据中的缩放后尺度计算角度
                img_width = img_metas[0]['img_shape'][1]  # 宽度在img_shape的第二个位置
                shift_angle = shift_pixels * (2 * PI) / img_width
                rolled_points[:, 0] = torch.remainder(rolled_points[:, 0] + PI + shift_angle, 2 * PI) - PI
                gt_points_move_view.append(rolled_points)
            else:
                gt_points_move_view.append(points)
        
        return img_move_view, img_metas_move_view, gt_bfov_move_view, gt_points_move_view
    ...
    def forward_train(self,
                      ...
                      gt_masks=None,
                      proposals=None,
                      is_move_view=True,  # 新增：是否启用滚动增强
                      move_shift_max_ratio=0.25,  # 新增：最大滚动比例阈值（相对于图像宽度）
                      **kwargs):
        ...
        # 滚动增强处理
        x_move_view = None
        img_metas_move_view = None
        gt_bfov_move_view = None
        gt_points_move_view = None
        
        if is_move_view:
            # 随机生成滚动像素数（基于最大比例阈值）
            if move_shift_max_ratio > 0:
                # 使用图像元数据中的缩放后尺度，而不是img.shape
                img_width = img_metas[0]['img_shape'][1]  # 宽度在img_shape的第二个位置
                max_shift = int(img_width * move_shift_max_ratio)
                move_shift_pixels = torch.randint(-max_shift, max_shift, (1,)).item()
            else:
                move_shift_pixels = 0  # 不滚动
            
            # 生成滚动视图
            img_move_view, img_metas_move_view, gt_bfov_move_view, gt_points_move_view = \
                self.roll_erp_image_and_annotations(img, img_metas, gt_bfov, gt_points, move_shift_pixels)
            
            # 提取滚动视图特征
            x_move_view = self.extract_feat(img_move_view)
        ...
        for stage in range(self.num_stages):
            if stage == 0:
                ...
                # 滚动视图的提案生成（如果启用）
                if is_move_view:
                    generate_proposals_move_view, proposals_valid_list_move_view = gen_proposals_from_cfg(
                        gt_points_move_view, base_proposal_cfg, img_meta=img_metas_move_view)
                    
                    dynamic_weight_move_view = torch.cat(gt_labels).new_ones(len(torch.cat(gt_labels)))
                    neg_proposal_list_move_view, neg_weight_list_move_view = None, None
                    pseudo_boxes_move_view = generate_proposals_move_view
            ...
            # 滚动视图训练和一致性损失计算（仅在阶段0且启用滚动增强时）
            if is_move_view and stage == 0:
                roi_losses_move_view, pseudo_boxes_move_view, dynamic_weight_move_view ,\
                    bbox_results_move_view = self.roi_head.forward_train(stage, x_move_view, img_metas_move_view,
                                                                       pseudo_boxes_move_view,
                                                                       generate_proposals_move_view,
                                                                       proposals_valid_list_move_view,
                                                                       neg_proposal_list_move_view, neg_weight_list_move_view,
                                                                       gt_true_bboxes, gt_labels,
                                                                       dynamic_weight_move_view,
                                                                       gt_bboxes_ignore, gt_masks,
                                                                       gt_bfov_move_view,
                                                                       gt_bboxes,
                                                                       gt_points_move_view,
                ...
                # 计算滚动一致性损失
                cls_sim, ins_sim, score_sim = self.calculate_move_view_consistency_loss(
                    bbox_results, bbox_results_move_view, proposals_valid_list_move_view, stage=0)
                
                # 使用SmoothL1Loss计算一致性损失
                loss_move_cls = 0.01 * self.loss_diff_view(cls_sim, torch.zeros_like(cls_sim))
                loss_move_ins = 0.02 * self.loss_diff_view(ins_sim, torch.zeros_like(ins_sim))
                
                # 添加到损失字典
                losses[f'stage{stage}_loss_MVC_cls'] = loss_move_cls  # Move View Consistency
                losses[f'stage{stage}_loss_MVC_ins'] = loss_move_ins
                
                # 添加滚动视图的MIL损失（可选）
                for key, value in roi_losses_move_view.items():
                    losses[f'stage{stage}_move_{key}'] = value

            
            # 打印bbox_results中的类别得分和实例得分
            # with open('./bfov/temp_result/bbox_results_dump.txt', 'a') as f:
            #     f.write(f"\n{'='*60}\n")
            #     f.write(f"Stage: {stage}\n")
                
            #     if isinstance(bbox_results, dict):
            #         # 只打印cls_score和ins_score
            #         for key in ['cls_score', 'ins_score']:
            #             if key in bbox_results:
            #                 value = bbox_results[key]
            #                 f.write(f"\nKey: {key}\n")
            #                 f.write(f"  Type: {type(value)}\n")
            #                 if hasattr(value, 'shape'):
            #                     f.write(f"  Shape: {value.shape}\n")
            #                 f.write(f"  Dtype: {getattr(value, 'dtype', 'N/A')}\n")
            #                 # 打印部分数据内容（前10个元素）
            #                 try:
            #                     if hasattr(value, 'numpy'):
            #                         arr = value.detach().cpu().numpy()
            #                         f.write(f"  First 10 elements:\n")
            #                         flat_arr = arr.flatten()[:10]
            #                         f.write(f"    {flat_arr}\n")
            #                     else:
            #                         f.write(f"  Value: {str(value)[:500]}...\n")
            #                 except Exception as e:
            #                     f.write(f"  Error printing content: {e}\n")
            #             else:
            #                 f.write(f"\nKey: {key} - Not found\n")
