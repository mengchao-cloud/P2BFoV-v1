...
class COCOExpandEval(COCOeval):
    ...
    def _prepare(self):
        ...
        p = self.params
        #所有的gt值都已经加载进来了，只需要计算iou的时候计算gt_bfov和dt_bbox即可
        if p.useCats:
    ...
    def evaluate(self):
        ...
        if p.iouType == 'segm' or p.iouType == 'bbox':
            # 改成我们自己的球面iou计算逻辑
            # computeIoU = self.computeIoU，原始逻辑
            computeIoU = self.spherical_iou_computeIoU
    ...
    def spherical_iou_computeIoU(self, imgId, catId):
        """
        Compute spherical IoU between detection results and ground truth.
        ...
        if p.iouType == 'segm':
            ...
            # 提取bbox检测（实际是bfov格式），形状：[D, 4]
            d = [d['bbox'] for d in dt]  # 提取bbox检测（实际是bfov格式）
