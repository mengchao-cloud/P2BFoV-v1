...
"""
GPU复用BFoV掩码生成器
基于GPU复用的高效掩码生成，支持随机BFoV参数生成和可视化
"""
...
def main():
    ...
    if specified_bfovs:
        print(f"\n2. 添加 {len(specified_bfovs)} 个指定BFoV参数...")
        specified_bfovs_rad = []
        ...
        # 将指定的BFoV参数转换为张量并合并到随机BFoV后面
        specified_tensor = torch.tensor(specified_bfovs_rad, dtype=torch.float32, device=device)
        bfov_tensor = torch.cat([bfov_tensor, specified_tensor], dim=0)
    ...
    # 4. 在空白图像上可视化（多种分辨率）
    print(f"\n4. 在多种分辨率的空白图像上可视化BFoV掩码和边界框...")
    output_dir = './bfov/result/mask_to_box_dif_fenbian/'
