...
# 输入输出配置
MERGED_COCO_FILE = './bfov/temp_json/merge_result/boxinferbfov/box_to_bfov.json'
IMAGE_DIR = '../360indoor/images/'
OUTPUT_DIR_BOX = './bfov/temp_json/result_image_box/'
OUTPUT_DIR_BFOV = './bfov/temp_json/result_image_bfov/'
...
os.makedirs(OUTPUT_DIR_BFOV, exist_ok=True)
...
def visualize_box_on_image(img, box_params, category_id=None):
    """
    在图像上可视化单个box
    
    参数:
    img: 输入图像
    box_params: box参数 [x, y, width, height]，像素坐标
    category_id: 类别ID
    ...
    # 绘制box中心点
    center_px = int(x + width / 2)
    center_py = int(y + height / 2)
    cv2.circle(img, (center_px, center_py), 3, (0, 255, 255), -1)
    
    # 绘制box标签
    cv2.putText(img, 'Box', (x1 + 10, y1 - 10), 
        ...
        for ann in annotations:
            ...
            if 'bbox' in ann:
                ...
                if len(box_params) == 4:
                    ...
                    img_box = visualize_box_on_image(img_box, box_params, category_id=category_id)
                    box_count += 1
            ...
            if 'bfov' in ann:
                ...
                if len(bfov_params) == 4:
                    ...
                    if score >= MIN_SCORE:
                        category_id = ann['category_id']
                        img_bfov = visualize_bfov_on_image(img_bfov, bfov_params, score=score, category_id=category_id)
                        bfov_count += 1
        
        # 保存box可视化结果
        output_path_box = os.path.join(OUTPUT_DIR_BOX, f"box_{file_name}")
        cv2.imwrite(output_path_box, img_box)
        
        # 保存BFOV可视化结果
        output_path_bfov = os.path.join(OUTPUT_DIR_BFOV, f"bfov_{file_name}")
        cv2.imwrite(output_path_bfov, img_bfov)
        
        processed_count += 1
        print(f"已处理图像 {file_name}，绘制了 {box_count} 个box和 {bfov_count} 个BFOV")
        print(f"- Box结果保存到: {output_path_box}")
        print(f"- BFOV结果保存到: {output_path_bfov}")
    
    print(f"\n批量可视化完成！")
    print(f"- 处理图像数: {processed_count}")
    print(f"- 错误图像数: {error_count}")
    print(f"- Box输出目录: {OUTPUT_DIR_BOX}")
    print(f"- BFOV输出目录: {OUTPUT_DIR_BFOV}")
