...
class CocoFmtDataset(CocoDataset):
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if ann.get('iscrowd', False):
                ...
                if 'bfov' in ann:  # add by mc
                    gt_bfov.append(ann['bfov'])
                if 'point' in ann:  # add by mc
        ...
        ann = dict(
            ...
            masks=gt_masks_ann,
            seg_map=seg_map,
            bfov=gt_bfov,  # add by mc
            point=gt_points,  # add by mc
        )
        if len(true_bboxes) > 0:  # add by hui
    ...
    def evaluate(self,
        ...
        for metric in metrics:
            ...
            try:
                ...
                if merge_after_infer_kwargs is not None:  # merge result before eval
                    ...
                    cocoGt, result_files[metric] = merge_det_result(result_files[metric], self.ann_file, merge_gt_file,
                                                                    merge_nms_th)
                ###################################################################
                # - 来源 ： model.simple_test() 方法返回的检测结果
                # - 形状 ： [N, num_classes, (m, 5)]
                            #不用调整配置文件，只需要将其获取gt的键值改成'bfov',暂时搁置
            ...
            # 创建评估的位置在这个地方，所以并没有用官方的COCOeval类
