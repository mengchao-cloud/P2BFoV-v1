...
def sph2pob_efficient(sph_gt, sph_pred, rbb_angle_version='deg', rbb_edge='arc', rbb_angle='equator'):
    ...
    plannar_gt = torch.cat([theta_g_, phi_g_, alpha_g_, beta_g_, angle_g_], dim=1)
    plannar_pred = torch.cat([theta_p_, phi_p_, alpha_p_, beta_p_, angle_p_], dim=1)
...
def standardize_rotated_box(rotated_gt, rotated_pred, rbb_angle='deg'):
    ...
    rotated_gt = torch.cat([theta_g_, phi_g_, alpha_g, beta_g, angle_g_], dim=1)
    rotated_pred = torch.cat([theta_p_, phi_p_, alpha_p, beta_p, angle_p_], dim=1)
